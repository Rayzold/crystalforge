...
- UI follow-up (commit `9f175c8`, both catalogues, standalone — no token bump): NPC cards get a **monogram disc + card accent tinted by class** (`CLASS_HUE` map + hashed fallback, `classHue`/`initials`), killing the text-wall. **GM Catalogue only**: the Ties field linkifies each `SL-#### (Name)` into a `.tie-link` button that `openNpc()`s that NPC — a clickable relationship web (`linkTies` helper; players.html deliberately has none).
- UI follow-up 2 (commit `41afe31`, both catalogues): **card↔list view toggle** (`model.view`, `catBody`/`npcRow`, persisted in `localStorage.sl_view` + `?view=list`), **CR magnitude chips** (`crBand`: low/mid/high/max → dim/cyan/gold/rose, on cards + rows), and **`/` focuses the search box**. Both catalogues stay in sync — apply edits to players.html AND gm-catalogue.html (the latter is a copy with the GM data layer + ties).
- UI follow-up 3 (commit `a6351ff`, **all three** scarred-lands pages): **starfield** (inline-SVG tile in the body `background-image`, `background-attachment:fixed`) + **region-tinted ambient** — the two body radial-glows read `hsla(var(--amb-1)…)` and `applyAmbient()` sets `--amb-1/2` to `REGION_HUE[region]` (+ `body.region-tinted{--amb-a:.22}`). Catalogues tint when exactly one region facet is picked (in `renderContent`); the roller always follows its `region` (in `drawRegions`+onclick). Region facet rows / roller buttons get a `.reg-dot`; NPC detail header gets a `linear-gradient` region banner. NOTE: top-level `const` in these classic-script pages is NOT on `window` — probe with the bare name, not `window.X`.
