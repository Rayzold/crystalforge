...
import { AdminConsole } from "./admin/AdminConsole.js?v=v1.7.24-20260907120000";
import { createCatalogEntryFromInput, getBuildingEmoji, getCatalogKey } from "./content/BuildingCatalog.js?v=v1.7.24-20260907120000";
import {
  APP_VERSION,
  ...
  SALARIES_LEVELS,
  SPEED_MULTIPLIERS
} from "./content/Config.js?v=v1.7.24-20260907120000";
import { EVENT_POOLS } from "./content/EventPools.js?v=v1.7.24-20260907120000";
import { RARITY_ORDER } from "./content/Rarities.js?v=v1.7.24-20260907120000";
import { GameState } from "./engine/GameState.js?v=v1.7.24-20260907120000";
import { installModalKeyboardHandlers } from "./engine/ModalFocus.js?v=v1.7.24-20260907120000";
import { downscaleImageFile, formatNumber } from "./engine/Utils.js?v=v1.7.24-20260907120000";
import { AnimationEngine, getManifestRevealTotalDuration } from "./fx/AnimationEngine.js?v=v1.7.24-20260907120000";
import { AudioEngine } from "./fx/AudioEngine.js?v=v1.7.24-20260907120000";
import { ensureFirebaseAuth, getFirebaseUserId } from "./firebase/FirebaseConfig.js?v=v1.7.24-20260907120000";
import {
  isFirebaseConfigured,
  loadFirebaseRealmState,
  saveFirebaseRealmState,
  subscribeFirebaseRealmState
} from "./firebase/FirebaseSharedState.js?v=v1.7.24-20260907120000";
import {
...
} from "./systems/BuildingSystem.js?v=v1.7.24-20260907120000";
import { addMonthsToOffset, dateFromParts, formatDate, getMonthStartOffset, getStructuredDate } from "./systems/CalendarSystem.js?v=v1.7.24-20260907120000";
import {
  addCitizens,
  applyCitizenBulkSet,
  ...
  resetCitizens,
  setCitizens
} from "./systems/CitizenSystem.js?v=v1.7.24-20260907120000";
import { recalculateCityStats } from "./systems/CityStatsSystem.js?v=v1.7.24-20260907120000";
import { addCrystals, setCrystals } from "./systems/CrystalSystem.js?v=v1.7.24-20260907120000";
import {
  ...
  pauseConstruction
} from "./systems/ConstructionSystem.js?v=v1.7.24-20260907120000";
import { resetDistrictLevels, setDistrictDefinition, setDistrictLevelOverride, getDistrictSummary } from "./systems/DistrictSystem.js?v=v1.7.24-20260907120000";
import {
  clearDecisionSnooze,
  ...
  setDecisionSnooze
} from "./systems/DecisionInboxSystem.js?v=v1.7.24-20260907120000";
import { setDriftEvolutionStageOverride, syncDriftEvolutionState } from "./systems/DriftEvolutionSystem.js?v=v1.7.24-20260907120000";
import { clearActiveEvents, triggerEvent } from "./systems/EventSystem.js?v=v1.7.24-20260907120000";
import {
  ...
  startExpedition
} from "./systems/ExpeditionSystem.js?v=v1.7.24-20260907120000";
import { VEHICLE_DEFINITIONS } from "./content/VehicleConfig.js?v=v1.7.24-20260907120000";
import {
  addBehemoth,
  addBehemothAbility,
  addBehemothUpkeep,
...
} from "./systems/BehemothSystem.js?v=v1.7.24-20260907120000";
import {
  addNpc,
  addNpcAbility,
  clearNpcImage,
  ...
  updateNpcStat,
  getCrafterCapacity
} from "./systems/NpcSystem.js?v=v1.7.24-20260907120000";
import {
  createCraftingItem,
  collectCraftingItem,
  deleteCraftingItem,
  ...
  pauseCraftingItem,
  resumeCraftingItem,
} from "./systems/CraftingSystem.js?v=v1.7.24-20260907120000";
import { findCraftingTemplate, CRAFTING_STATIONS, craftingTemplateCategory, describeCraftingStationBonuses } from "./ui/CraftingPage.js?v=v1.7.24-20260907120000";
import { addCooldown, removeCooldown, restartCooldown, markCooldownTriggered, ageCooldown, isCooldownReady, getCooldownReadyDay } from "./systems/CooldownSystem.js?v=v1.7.24-20260907120000";
import { generateMonthWeather } from "./systems/WeatherSystem.js?v=v1.7.24-20260907120000";
import { craftingCompletionDay } from "./systems/CraftingSystem.js?v=v1.7.24-20260907120000";
import {
  addAwakened,
  clearAwakenedImage,
  removeAwakened,
  ...
  updateAwakenedField
} from "./systems/AwakenedSystem.js?v=v1.7.24-20260907120000";
import {
...
} from "./systems/PlayerCharacterSystem.js?v=v1.7.24-20260907120000";
import { getDailyCitySnapshot } from "./systems/CitySnapshotSystem.js?v=v1.7.24-20260907120000";
import { manifestSelectedRarity } from "./systems/GachaSystem.js?v=v1.7.24-20260907120000";
import { addHistoryEntry } from "./systems/HistoryLogSystem.js?v=v1.7.24-20260907120000";
import {
  canPlaceBuildingAt,
...
} from "./systems/MapSystem.js?v=v1.7.24-20260907120000";
import { addShards, convertShardsToCrystals, setShards } from "./systems/ShardSystem.js?v=v1.7.24-20260907120000";
import {
...
} from "./systems/StorageSystem.js?v=v1.7.24-20260907120000";
import { advanceTime, advanceTimeByDays } from "./systems/TimeSystem.js?v=v1.7.24-20260907120000";
import { applyCompletedGoalRewards } from "./systems/GoalSystem.js?v=v1.7.24-20260907120000";
import { forceTownFocus, getMayorAdvice, reopenTownFocusSelection, selectTownFocus, updateTownFocusAvailability } from "./systems/TownFocusSystem.js?v=v1.7.24-20260907120000";
import { getEmergencyStatus, getCityTrendSummary, setResourceValue } from "./systems/ResourceSystem.js?v=v1.7.24-20260907120000";
import { Toasts } from "./ui/Toasts.js?v=v1.7.24-20260907120000";
import { getDefaultTownFocusPreviewId } from "./ui/TownFocusShared.js?v=v1.7.24-20260907120000";
import { UIRenderer } from "./ui/UIRenderer.js?v=v1.7.24-20260907120000";
import { buildSearchIndex, filterSearchIndex, renderSearchResults } from "./ui/GlobalSearch.js?v=v1.7.24-20260907120000";
import { createBlankPlayerCharacter, createBlankWealthItem } from "./ui/EquipmentSheetPage.js?v=v1.7.24-20260907120000";
...
function setResources(nextResources) {
  ...
  // Apply only the keys present in nextResources, and only when the parsed
  // value is finite. Missing or invalid keys keep the current value — this
  // matters for the per-resource Apply button, which submits a single-key
  // patch and would otherwise overwrite the other five with NaN.
  const applyKey = (draft, key) => {
    if (!(key in nextResources)) return;
    const numeric = Number(nextResources[key]);
    if (!Number.isFinite(numeric)) return;
    draft.resources[key] = Math.max(0, numeric);
  };
  commit((draft) => {
    applyKey(draft, "gold");
    applyKey(draft, "food");
    applyKey(draft, "materials");
    applyKey(draft, "salvage");
    applyKey(draft, "mana");
    applyKey(draft, "prosperity");
  ...
  const changedKeys = ["gold", "food", "materials", "salvage", "mana", "prosperity"].filter((key) => key in nextResources);
  if (changedKeys.length) markRecentResourceChanges(changedKeys);
...
// Firestore caps a single document at 1 MB. Uploaded portraits/building art are
// stored inline as base64 data URLs, so a realm with lots of custom art can blow
// past that and make Cloud Save fail. If the payload is too big we drop the
// inline image blobs from the published copy (local saves keep them) and report
// it, instead of letting the write silently fail.
const FIREBASE_DOC_SOFT_LIMIT_BYTES = 900_000;

function stripInlineImageData(serializable) {
  let stripped = 0;
  const collections = [
    serializable.buildings,
    serializable.npcs,
    serializable.awakened,
    serializable.behemoths,
    serializable.uniqueCitizens,
    serializable.playerCharacters
  ];
  for (const collection of collections) {
    if (!Array.isArray(collection)) continue;
    for (const entry of collection) {
      if (entry && typeof entry.imageData === "string" && entry.imageData.startsWith("data:image/")) {
        entry.imageData = "";
        stripped += 1;
      }
    }
...
async function publishSharedStateToFirebase(realmId, state = getCurrentState()) {
  ...
  if (JSON.stringify(serializable).length > FIREBASE_DOC_SOFT_LIMIT_BYTES) {
    imagesStripped = stripInlineImageData(serializable);
  }
...
const actions = {
  ...
  async loadFirebaseRealm() {
    ...
    try {
      const result = await publishSharedStateToFirebase(getPublishedFirebaseRealmId());
      if (result?.imagesStripped > 0) {
        reportSuccess(`Cloud save updated, but ${result.imagesStripped} uploaded image(s) were dropped to fit the cloud size limit. Use a local save to keep custom art.`, "publish");
      } else {
...
// Mirror the Drift's current region into shared same-origin localStorage so the
// Scarred Lands roller and World Catalogue (both under /scarred-lands/) can follow it.
function mirrorDriftRegion(state) {
  try {
    const driftRegion = state.settings.driftRegion ?? "";
    if ((localStorage.getItem("sl_drift_region") ?? "") !== driftRegion) {
      if (driftRegion) {
        localStorage.setItem("sl_drift_region", driftRegion);
      } else {
        localStorage.removeItem("sl_drift_region");
      }
    }
  } catch (error) {
    /* localStorage unavailable — the cross-tool bridge simply stays inert */
  }
}
...
root.addEventListener("click", async (event) => {
  ...
  switch (action) {
    ...
    case "reset-codex-filter":
      ...
      break;
    case "toggle-codex-reveal-names":
      renderer.setTransientUi(
        { codexRevealNames: !renderer.transientUi.codexRevealNames },
        getCurrentState()
      );
      break;
    ...
    case "toggle-resource-quick-edit": {
      renderer.setTransientUi({ resourceQuickEdit: !renderer.transientUi.resourceQuickEdit }, getCurrentState());
      break;
    }
    case "quick-adjust-crystal": {
      const rarity = target.dataset.rarity ?? "";
      const delta = Number(target.dataset.delta);
      if (!rarity || !Number.isFinite(delta) || delta === 0) {
        break;
      }
      commit((draft) => {
        addCrystals(draft, rarity, delta);
      });
      break;
    }
    case "set-policy-salaries": {
      const level = Number(target.dataset.level);
      if (!SALARIES_LEVELS.includes(level)) {
        break;
      }
      commit((draft) => {
        if (!draft.policySettings) draft.policySettings = {};
        draft.policySettings.salariesLevel = level;
      });
      break;
    }
    case "set-policy-effort": {
      const level = Number(target.dataset.level);
      ...
      commit((draft) => {
        if (!draft.policySettings) draft.policySettings = {};
        draft.policySettings.effortLevel = level;
      });
      break;
    }
      ...
      renderer.setTransientUi(
        {
          expeditionDraft: {
            ...
            typeId: target.dataset.typeId ?? "rescue",
            missionId: null
            // Same reasoning as set-expedition-mission: preserve team /
            // awakenedIds / resources / vehicleId across type switches.
          }
        },
        getCurrentState()
      );
    ...
    case "max-expedition-resource":
      ...
      break;
    case "toggle-expedition-awakened": {
      void audioEngine.playUiAccent("soft");
      const awakenedId = target.dataset.awakenedId ?? "";
      if (!awakenedId) {
        break;
      }
      const current = Array.isArray(renderer.transientUi.expeditionDraft?.awakenedIds)
        ? renderer.transientUi.expeditionDraft.awakenedIds
        : [];
      const next = current.includes(awakenedId)
        ? current.filter((id) => id !== awakenedId)
        : [...current, awakenedId];
      renderer.setTransientUi(
        {
          expeditionDraft: {
    ...
    case "export-chronicle-notes":
      ...
      break;
    case "randomize-month-weather": {
      void audioEngine.playUiAccent("soft");
      const monthOffset = Number(target.dataset.monthOffset);
      if (Number.isFinite(monthOffset)) {
        commit((draft) => {
          generateMonthWeather(draft, monthOffset);
        });
        reportSuccess("Weather rolled for the displayed month.");
      }
      break;
    }
    case "go-to-problem":
...
root.addEventListener("change", (event) => {
  ...
  if (target.dataset.action === "set-resource-value") {
    ...
    commit((draft) => {
      setResourceValue(draft, key, numeric);
    });
  }
  ...
  if (target.dataset.action === "set-drift-region") {
    commit((draft) => {
      draft.settings.driftRegion = (target.value ?? "").trim() || null;
    });
  }
...
mirrorDriftRegion(getCurrentState());
renderer.render(getCurrentState());
applyMapViewPreview();
